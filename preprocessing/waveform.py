...
class WaveformPreprocessing:
    def __init__(self, wv_config={}, db_config_path="YOUR OWN DB PATH"):
        """
        wv_config : {"WAVE_DURATION_SECONDS": 10, "INPUT_INTERVAL_MINUTES":10, "INPUT_DURATION_LIMIT_HOURS":4,
        ...
        Returns:
            pd.DataFrame: waveform data (before preprocessing)
                e.g.
                   ...
                   YYYY-MM-DD HH:MM:SS  YYYY-MM-DD HH:MM:SS   II   34.16,41.48,46.36,46.36,48.8,56.12,65.88,70.76...
        """
        db_query = f"""SELECT stime, etime, param, data
                FROM {self.db_config["Database"]["schema"]}.waveform_data
                WHERE session_id='{sr["session_id"]}'
                AND stime BETWEEN TO_TIMESTAMP('{sr["event_timestamp"]}' , 'YYYY-MM-DD hh24:mi:ss')::timestamp - INTERVAL '{self.wv_config["WAVE_DURATION_SECONDS"]+6} seconds'
                AND TO_TIMESTAMP('{sr["event_timestamp"]}' , 'YYYY-MM-DD hh24:mi:ss')::timestamp
                AND param in ('{"','".join([i for i in self.wv_config["raw_wave_name_list"] if i!=excluding_param])}')
                AND data not like '"[%'
                AND data not like '"{{%'
                """
