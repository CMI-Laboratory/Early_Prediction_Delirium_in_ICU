...
class VitalsPreprocessing:
    def __init__(self, vit_config={}, db_config_path="config/db_config.yaml"):
        """
        # Extraction data from db each 10 minutes within 4hours from cam-icu
    ...
    def fetch_vital_from_db(self, sr: pd.Series):
        ...
        """
        db_query = f'SELECT * from Vital_DB'
                """
    ...
    def get_demo_raw_vital(self, sr):
        ...
        raw_vital = raw_vital.query(f'unique_id=="{sr["unique_id"]}"')
        raw_vital = raw_vital[
            raw_vital["mtime"]
            >= (
                event_timestamp
        ...
        raw_vital = raw_vital.drop(columns=["unique_id"])
