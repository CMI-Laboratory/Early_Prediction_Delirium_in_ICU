...
class VitalsPreprocessing:
    def __init__(self, vit_config={}, db_config_path="config/db_config.yaml"):
        """
    ...
    def fetch_vital_from_db(self, sr: pd.Series):
        ...
        """
        db_query = f"""SELECT mtime, param, data
                FROM {self.db_config["Database"]["schema"]}.numeric_data
                WHERE session_id='{sr["session_id"]}'
                AND mtime BETWEEN TO_TIMESTAMP('{sr["event_timestamp"]}' , 'YYYY-MM-DD hh24:mi:ss')::timestamp - INTERVAL '{self.vit_config["INPUT_DURATION_LIMIT_HOURS"]} hours'
                AND TO_TIMESTAMP('{sr["event_timestamp"]}' , 'YYYY-MM-DD hh24:mi:ss')::timestamp
                AND param in ('{"','".join(self.vit_config["raw_vital_name_list"])}')
    ...
    def get_demo_raw_vital(self, sr):
        ...
        raw_vital = pd.read_csv(
            "/VOLUME/project/saveu/save-u-d_deployment/clinical_trial/data/sample/raw_data/sample_vitals.csv",
            parse_dates=["mtime"],
        )
